...
import xml.patent.serice.patent.service.dto.RangeDTO;
import xml.patent.serice.patent.service.dto.ReportDTO;
import xml.patent.serice.patent.service.jaxb.ReportMarshaller;
...
public class ReportService {
    ...
    public ReportDTO getReportForRange(RangeDTO rangeDTO) throws URISyntaxException {
        ...
        URI uri = new URI(basicUrl + "/report/patent");
    ...
    public int getNumberOfReportsForRange(RangeDTO rangeDTO) throws Exception {
        List<PatentRequest> requestList = existManager.retrieveCollection();
        int numberOfPatents = 0;
        for (PatentRequest patentRequest : requestList){
            if (patentRequest.getPatentData().getApplicationDate().getDate().isBefore(rangeDTO.getStartDateAsDate()))
                continue;
            if (patentRequest.getPatentData().getApplicationDate().getDate().isAfter(rangeDTO.getEndDateAsDate()))
                continue;
            numberOfPatents++;
        }
    ...
    private FopFactory fopFactory= FopFactory.newInstance(new File("src/fop.xconf"));
    ...
    public String generateReportPDF(RangeDTO rangeDTO) throws Exception {
        ...
        // */
        return "http://localhost:8082/resources/report/" + documentName + ".pdf";
    }
