...
import xml.authorship.service.authorship.service.dto.FilterDTO;
...
public class FusekiReader {
    ...
    private static final String GRAPH_URI = "/authorship/metadata";
...
//    }
    ...
    public HashSet<String> search(List<FilterDTO> elements) {
        ...
        ResultSet results = query.execSelect();
        HashSet<String> foundAuthorshipReqs = new HashSet<>();
        while (results.hasNext()) {
            QuerySolution querySolution = results.next();
            String[] authorshipUrl = querySolution.getResource("authorship").toString().split("/");
            foundAuthorshipReqs.add(authorshipUrl[authorshipUrl.length - 1]);
        }
        System.out.println("***********************************");
        for (String founds : foundAuthorshipReqs) {
            System.out.println(founds);
        }
        query.close();
        return foundAuthorshipReqs;
    }
}
